<template>
    <view class="page">
        ...
        <view v-if="adress.length > 0" class="adress" @tap="to_adress">
            ...
            <view>
                <view class="font-30 ellipsis">{{ adress[0].status }}</view>
                <view class="font-26">{{ adress[0].time }}</view>
        ...
        <view class="adress">
            ...
            <view class="adress-right">
                ...
                <view class="ellipsis-two font-26">收货地址：{{ order_info.address1 }}{{ order_info.address }}</view>
            </view>
        </view>
        <view class="good-list">
            ...
            <view class="order-good">
                <view class="order-item" v-for="(item, index) in order_info.goods_list" :key="index" @tap="to_good_detail(item)">
                    ...
                    <view class="good-item">
                        ...
                        <view class="font-26">{{ item.spec_key_name }}</view>
                        <view class="flex flex2">
                            <view class="font-32">￥{{ item.member_goods_price }}</view>
                            <view
                                class="font-26 refund"
                                v-if="order_info.return_btn == 1 && item.is_send < 2 && order_info.order_prom_type == 0 && order_info.order_prom_id == 0"
                                @tap.stop="to_return_btn(item)"
                            >
                                售后
                            </view>
                        </view>
        ...
        <view class="other-price">
            ...
            <view>
                <view v-if="order_info.note">发货备注</view>
                <view>{{ order_info.note }}</view>
            </view>
        </view>
        ...
        <view class="create-time font-26">
            ...
            <view v-if="order_info.pay_status == 1">付款时间：{{ order_info.pay_time }}</view>
            <view v-if="order_info.shipping_status == 1">发送时间：{{ order_info.shipping_time }}</view>
        </view>
        <view class="fixed font-30" v-if="get_show">
            <view v-if="order_info.cancel_btn == 1 && order_info.pay_status != 1" @tap="cancel(order_info)">取消订单</view>
            <view
                class="option-1"
                v-if="order_info.cancel_btn == 1 && order_info.pay_status == 1 && order_info.order_prom_type == 0 && order_info.order_prom_id == 0"
                @tap="cancel_pay()"
            >
                取消订单
                <!-- 已支付 -->
            </view>
            <view class="option-2" v-if="order_info.pay_btn === 1 && order_info.pay_code != 'offline_pay' && order_info.pay_doc_status == 0" @tap="to_pay">立即付款</view>
            <view class="option-2" v-if="order_info.remind_btn == 1" @tap="remind(order_info)">提醒发货</view>
            <!-- <view class="option-2" v-if="item.commission_btn==1" @tap="showCom(item)">确认分发</view> -->
            <view class="option-2" v-if="order_info.commission_btn == 1" @tap="showCom(order_info)">确认分发</view>
            <view class="option-2" v-if="order_info.receive_btn == 1" @tap="order_confirm(order_info)">确认收货</view>
        </view>
        <cancel-dialog v-model="cancel_dialog" :id="id" @check="get_detail"><!-- 取消订单 --></cancel-dialog>
        <pay-method
            v-model="is_pay"
        ...
        <no-password v-model="no_password" @check="set_password"><!-- 还没设置密码 --></no-password>
        <pay-password ref="pay" :pay_price="order_info.order_amount" v-model="pay_password" @check_word="check_word"><!-- 输入密码 --></pay-password>
...
export default {
    ...
    methods: {
        ...
        over_pay() {
            //余额支付
            this.$http('get|api/User/is_paypwd')
                .then(res => {
        ...
        other_pay(e) {
            ...
            // console.log(e, this.$store.state)
            this.$http('get|api/joinWxPay/pay', {
                order_id: this.id
            }).then(res => {
                let code = res.result
