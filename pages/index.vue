<template>
	<view>
		...
		<swiper class="swiper" :indicator-dots="true" :autoplay="true" indicator-color="rgba(255,255,255,0.3)"
			...
			<swiper-item v-for="(item,index) in swiper" :key="index" @tap="to_page(item)">
		...
		<view class="placard" @tap="to_placard">
			...
			<view class="content font-26">
				<view :style="{transform: `translateY(-${news_index*34}rpx)`}">
					<view v-for="(item,index) in news" :key="index" class="ellipsis">
						{{item.title}}
					</view>
				</view>
			</view>
			...
			<view class="good-box">
				<view v-for="(item,index) in scroll_good" :key="index" class="good-item" @tap="to_good_detail(item)">
					...
					<view class="price font-34 bold">
						...
						<view class="old-price font-24 inline">
							￥{{item.shop_price}}
						</view>
					</view>
					<view class="xiangou font-24">
						限购{{item.buy_limit}}件
					</view>
				</view>
			</view>
		...
		<!-- #ifdef H5 -->
		<view class="new-bg" v-if="newDialog&&switchs.h5_download_switch == 1" @touchmove.stop.prevent
			@tap="newDialog = false">
			<view>
				<view class="h5-poster">
					<view class="box">
						<image :src="$image + switchs.h5_download"></image>
					</view>
				</view>
		...
		<!-- #ifdef APP-PLUS -->
		<view class="new-bg" v-if="newDialog&&switchs.download_switch == 1" @touchmove.stop.prevent
			@tap="newDialog = false">
			<view>
				<view class="h5-poster">
					<view class="box">
						<image :src="$image + switchs.download"></image>
					</view>
				</view>
				<!-- <image class="close-dialog" src="/static/close_dialog.png" @tap="newDialog = false"></image> -->
			</view>
	...
	export default {
		...
		async onLoad() {
			...
			if (obj.code) {
				...
				this.$http('get|api/MobileBase/checkLogin').then(res => {
					//没登陆
				}).catch(e => {
					this.$http('get|api/User/checkUserinfo').then(res => {
						this.$nextTick(() => {
		...
		methods: {
			...
			init() {
				this.$http('get|api/Index/index').then(res => {
					...
					this.adv = [...res.result.adv.map(row => {
						row.ad_code = this.$image + row.ad_code;
						return row
