<template>
    <view class="pay-account-back" v-if="dialog">
        <view class="content">
            ...
            <view class="header font-32">选择收款账户</view>
            <view @tap="to_active(item)" :class="['list-item', item.active ? 'check' : 'no_check']" v-for="(item, index) in list" :key="index">
                ...
                <image class="image-44" src="/static/paypal.jpeg" mode="" v-if="item.bank_type === 3" />
                <view class="font-30" v-if="item.bank_type === 3">PayPal: {{ item.bank_card }}</view>
            </view>
