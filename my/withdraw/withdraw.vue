	...
	<view class="content-box">
		<view class="option">
			<template v-for="(item, index) in option">
				<view v-if="item.open" @tap="to_active(item, index)" :key="index"
					:class="[item.active ? 'active' : '']">{{ item.label }}</view>
			</template>
		</view>
		<view class="from">
			...
			<view class="account-input-box">
				...
				<input class="account-input inline" @blur="handleUserMoney" v-model="option[option_active].from.money"
