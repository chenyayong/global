<template>
    <view>
        <view class="from">
            ...
            <view class="account-input-box"><input class="account-input" v-model="mobile" type="number" placeholder="请输入到账账号" @blur="get_account" /></view>
            <view class="font-24" v-if="account">{{ account }}</view>
            <view class="font-30" style="margin-top:50rpx">转账数量</view>
            <view class="account-input-box">
                <view class="font-48">￥</view>
                <input class="account-input inline" v-model="from.number" type="number" placeholder="请输入转账积分" />
            </view>
            <view class="font-26 flex">
                <view>可转账积分{{ user_money }}</view>
                <view class="all-with" @tap="all_num">全部转账</view>
            </view>
            <view class="buttom font-32" @tap="submit">确定</view>
        </view>
...
export default {
    ...
    async onShow() {
        await this.$http('get|api/User/is_paypwd')
            ...
            .catch(e => {
                // console.log('get|api/User/is_paypwd', e)
                this.$http('get|api/User/transfer_points').then(res => {
    ...
    methods: {
        submit() {
            ...
            if (!+this.from.number >= 1) {
                this.$toastApp('转账数量最少为1')
                return
            }
            if (this.from.number != parseInt(this.from.number)) {
                this.$toastApp('转账数量为整数')
                return
            }
        ...
        save(e) {
            ...
            this.$http('post|api/User/transfer_points_info', this.from)
                .then(res => {
        ...
        get_account() {
            this.$http('post|api/User/transfer_points', {
                mobile: this.mobile
            })
                .then(res => {
                ...
                .catch(e => {
                    this.account = e.msg
                    // console.log('get_account error', e)
                })
        },
        all_num() {
