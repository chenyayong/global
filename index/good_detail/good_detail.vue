	...
	<view class="page">
		<view class="plan">
			<swiper class="swiper" :indicator-dots="true" :autoplay="true" :interval="5000" :duration="1000">
				<swiper-item v-for="(item,index) in goods_images_list" :key="index">
					<image :src="$image+item.image_url" />
				</swiper-item>
			</swiper>
		...
		<view v-if="is_snap" class="snap">
			...
			<view class="price-right">
				<view class="right-top font-20">
					已有{{virtual_num}}人购买
				</view>
				<view class="is-old-price font-20">
					￥{{goods.market_price}}
				</view>
			</view>
			<view class="snap-right">
				<view class="font-26 bold">
					距结束仅剩
				</view>
				<view class="flex">
					<view class="time-color">
						<view class="value font-22">00</view>
					</view>
		...
		<view class="price font-44 bold" v-if="!is_snap&&!group_buy">
			￥{{item_price}}
		</view>
		...
		<view class="service" v-if="service">
			<view class="service-content">
				...
				<view class="service-buttom font-30" @tap="service=false">
					知道了
				</view>
			</view>
		</view>
		<!-- 规格 -->
		<specification :type="type" :num="num" :src="$image+goods.original_img" :spec_goods_price="spec_goods_price"
			:price="item_price" :open="goods.is_upgrade!==1" v-model="specification" @to_active="item_active"
			@add="add_num" @add_card="add_card" @to_pay="to_pay">
		</specification>
	...
	export default {
		...
		async onLoad({
			...
			this.$http("get|api/Goods/goodsInfo", {
				...
				this.collect = res.result.collect === 1 ? true : false;
				let invite = uni.getStorageSync('setinvite');
				let level = uni.getStorageSync('level');
				// #ifdef H5
				let url = window.location.href.split('#')[1];
				let origin = window.location.origin + '/h5/#' +
					`/index/good_detail/good_detail?id=${this.id}` + `&invite=${invite||''}`;
